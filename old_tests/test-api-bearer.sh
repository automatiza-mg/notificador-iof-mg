#!/bin/bash
# Teste usando header Authorization Bearer (padrão HTTP)

echo "=== Teste com Authorization Bearer ==="
echo ""
...
echo "2. Testando /api/tasks/process-daily SEM Authorization (deve retornar 401):"
curl -s -w "\nHTTP: %{http_code}\n" -X POST http://localhost:8000/api/tasks/process-daily \
  -H "Content-Type: application/json" \
  -d '{"date": "2026-01-14}'
echo ""

echo "3. Testando /api/tasks/process-daily COM Authorization Bearer:"
RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" \
  -X POST http://localhost:8000/api/tasks/process-daily \
  -H "Content-Type: application/json" \
...
if [ "$HTTP_CODE" = "200" ]; then
    ...
    echo "⚠ HTTP 400 - Bad Request (pode ser problema com header ainda)"
