...
# Força o urllib3 (usado pelo requests/msal) a ignorar IPv6,
# evitando travamentos de 30s no macOS por DNS hole.
urllib3_cn.HAS_IPV6 = False


def _decode_id_token_claims(id_token: str) -> dict[str, Any]:
    """Decodifica o payload do JWT id_token (claims).
    Sem validação de assinatura no MVP.
    """
    try:
...
def login_view() -> Any:
    """Página de login: apenas botão Entrar com Microsoft.
...
def entra_login_view() -> Any:
    """Inicia o fluxo OIDC: guarda next, inicia auth code flow,
    redireciona para Entra.
    """
    if current_user.is_authenticated:
        next_url = request.args.get("next") or "/"
        return redirect(next_url)
    ...
    # MSAL reserva openid, profile, offline_access e adiciona automaticamente;
    # não passá-los
    ...
    app_msal = msal.ConfidentialClientApplication(
        ...
        authority=authority,
        timeout=10,  # Previne travamento em DNS IPv6 defeituoso no host
    )
    flow = app_msal.initiate_auth_code_flow(scopes=scopes, redirect_uri=redirect_uri)
    if not flow:
        return redirect(
            url_for("web.login", error="Falha ao iniciar login com Microsoft")
        )

    auth_uri = flow.get("auth_uri") or flow["auth_uri"]
    auth_uri += (
        "&prompt=select_account" if "?" in auth_uri else "?prompt=select_account"
    )
    ...
    # Resposta 200 com redirect via HTML/JS para o cookie de sessão ser gravado
    # antes de ir à Microsoft; em 302 para domínio externo o browser pode não
    # persistir o Set-Cookie
    return render_template("redirect_to_entra.html", auth_uri=auth_uri)


def auth_callback_view() -> Any:
    """Callback OIDC: troca code por tokens, provisiona usuário se
    necessário, login_user.
    """
    ...
    if not flow:
        # Sem code = volta do logout da Microsoft
        # (post_logout_redirect_uri aponta para /auth/callback)
    ...
    client_id = current_app.config.get("ENTRA_CLIENT_ID") or ""
    client_secret = current_app.config.get("ENTRA_CLIENT_SECRET") or ""
    current_app.config.get("ENTRA_REDIRECT_URI") or ""
    ...
    if not id_token:
        return redirect(
            url_for("web.login", error="Resposta da Microsoft sem id_token")
        )
    ...
    if not oid:
        return redirect(
            url_for("web.login", error="Identificador do usuário não encontrado")
        )
    ...
    user = (
        ...
        .filter_by(auth_provider="entra", external_subject=oid)
        .first()
    )
...
def logout_view() -> Any:
    ...
    Usa ENTRA_REDIRECT_URI (callback) como post_logout_redirect_uri para funcionar
    com apenas uma URI registrada no Azure; o callback redireciona para /login.
    """
    logout_user()
    session.clear()
    ...
    if authority and redirect_uri:
        ...
        redirect_target = (
            f"{authority}{logout_path}?post_logout_redirect_uri={post_logout}"
        )
    else:
...
def register(bp: Blueprint) -> None:
    ...
    bp.add_url_rule("/login", endpoint="login", view_func=login_view, methods=["GET"])
    bp.add_url_rule(
        "/auth/entra/login",
        endpoint="entra_login",
        view_func=entra_login_view,
        methods=["GET"],
    )
    bp.add_url_rule(
        "/auth/callback",
        endpoint="auth_callback",
        view_func=auth_callback_view,
        methods=["GET"],
    )
    bp.add_url_rule(
        "/logout", endpoint="logout", view_func=logout_view, methods=["GET", "POST"]
    )
