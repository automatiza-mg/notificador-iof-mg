...
def verify_api_key():
    ...
    if not api_key:
        return False, "API Key não fornecida. Use ?api_key=... na URL ou header Authorization: Bearer ..."
    ...
    try:
        ...
        try:
            ...
            if 'not found' in str(e).lower():
                current_app.logger.info(f"Nenhum diário encontrado para {publish_date}, pulando...")
                return
            raise
        ...
        try:
            ...
            for config in configs:
                try:
                    ...
                    current_app.logger.info(f"Notificação processada para config {config.id}")
                except Exception as e:
                    current_app.logger.error(f"Erro ao processar notificação para config {config.id}: {e}")
    ...
    try:
        ...
        if config.mail_to:
            ...
            try:
                ...
                current_app.logger.info(f"Email enviado para {config.mail_to} (config {config_id}){csv_info}")
            except Exception as e:
