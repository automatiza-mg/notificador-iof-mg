...
def upgrade() -> None:
    ...
    if "search_configs" not in existing_tables:
        op.create_table(
            ...
            sa.Column(
                "attach_csv", sa.Boolean(), nullable=False, server_default="false"
            ),
            sa.Column(
                "mail_to", sa.Text(), nullable=False, server_default="[]"
            ),  # JSON array como texto
            sa.Column(
                "mail_subject", sa.String(length=255), nullable=False, server_default=""
            ),
    ...
    if "search_terms" not in existing_tables:
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["search_config_id"], ["search_configs.id"], ondelete="CASCADE"
            ),
        )
    ...
    if "search_terms" in tables_after:
        try:
            ...
            existing_indexes = [
                idx["name"] for idx in inspector.get_indexes("search_terms")
            ]
            if "search_terms_config_idx" not in existing_indexes:
                op.create_index(
                    "search_terms_config_idx", "search_terms", ["search_config_id"]
                )
            ...
            try:
                result = bind.execute(
                    sa.text(
                        ...
                        "WHERE type='index' AND name='search_terms_config_idx'"
                    )
                ).fetchone()
                if not result:
                    op.create_index(
